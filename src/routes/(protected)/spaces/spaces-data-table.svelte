<script lang="ts">
	...
	let canBulkDelete = $derived(deleteConfirmText.toLowerCase() === 'delete');
...
<div class="mb-6 flex flex-wrap items-center gap-4">
	...
	<div class="relative flex-1 max-w-sm">
		...
		<Input placeholder="Search spaces..." bind:value={globalFilter} class="pl-10" />
	</div>
...
<AlertDialog.Root bind:open={deleteDialogOpen} onOpenChange={(open) => { if (!open) deleteConfirmText = ''; }}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			<AlertDialog.Title class="text-destructive">Delete {selectedCount} space{selectedCount > 1 ? 's' : ''} permanently?</AlertDialog.Title>
			<AlertDialog.Description>
				This action is <strong class="text-foreground">irreversible</strong>. All forms, submissions, and uploaded files
				in the selected space{selectedCount > 1 ? 's' : ''} will be permanently deleted.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<div class="py-4 space-y-2">
			<Label for="bulk-delete-confirm">Type <strong class="font-mono text-foreground">delete</strong> to confirm:</Label>
			<Input
		...
		<AlertDialog.Footer>
			...
			<AlertDialog.Action
				...
				disabled={isDeleting || !canBulkDelete}
