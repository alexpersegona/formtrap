...
import { canAddUser } from '$lib/server/pricing/limits';
...
export const actions = {
	default: async ({ request, locals, params }) => {
		...
		// Check user limit for this space
		const userCheck = await canAddUser(spaceId, userId);
		if (!userCheck.allowed) {
			return fail(403, {
				email,
				role,
				errors: {
					general: userCheck.reason || 'Cannot add more users to this space'
				}
			});
		}
