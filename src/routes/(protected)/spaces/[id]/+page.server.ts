...
import { organization, member, invitation, user } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	// Get forms from the appropriate DB (user's own or platform)
	...
	// Look up form creators from platform DB (user table is always in platform DB)
	const creatorIds = [...new Set(forms.map((f) => f.createdBy))];
	...
	// Attach unread counts and creator info to each form
	const formsWithCounts = forms.map((f) => ({
		...f,
		unreadCount: unreadMap.get(f.id) || 0,
		createdByUser: creatorMap.get(f.createdBy) || { name: 'Unknown', email: '', image: null }
	}));

	// Check form creation limits (only applies to free trial users)
	...
	if (freeTrial) {
		...
		if (currentFormCount >= PLAN_LIMITS.free.maxFormsPerSpace) {
