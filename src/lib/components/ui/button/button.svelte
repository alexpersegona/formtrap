<script lang="ts" module>
	...
	export const buttonVariants = tv({
		...
		variants: {
			variant: {
				default:
					'bg-primary text-primary-foreground text-shadow-sm shadow-xs hover:shadow-[0_0_4px_oklch(0.57_0.20_150/0.4)] dark:hover:shadow-[0_0_7px_oklch(93%_0.24_155/0.5)]',
				destructive:
				...
				primary_outline:
					'bg-background/50 text-primary shadow-xs hover:bg-primary/10 dark:bg-input/30 dark:hover:bg-primary/20 border border-primary',
				...
				original:
					'original-border overflow-visible rounded-lg border-0 text-foreground shadow-lg hover:scale-[103%] hover:shadow-xl transition-all duration-300'
			},
			size: {
...
{#if href}
	<a
		bind:this={ref}
		data-slot="button"
		class={cn(buttonVariants({ variant, size }), shadow && 'shadow-lg hover:shadow-lg', className)}
...
<style>
	...
	/* Original variant: animate the conic-gradient angle via @property */
	...
	@keyframes original-rotate {
		to {
			--original-angle: 360deg;
		}
	}

	:global(.original-border::before) {
		content: '';
		position: absolute;
		inset: -1px;
		z-index: -2;
		...
		background: conic-gradient(
			from var(--original-angle),
			transparent 0%,
			transparent 50%,
			oklch(from var(--primary) calc(l + 0.1) calc(c * 1.2) h / 0.3) 62%,
			oklch(from var(--primary) calc(l + 0.2) calc(c * 1.3) h / 0.6) 72%,
			oklch(from var(--primary) calc(l + 0.3) calc(c * 1.4) h / 0.9) 82%,
			oklch(from var(--primary) calc(l + 0.4) calc(c * 1.5) h / 1) 90%,
			oklch(from var(--primary) calc(l + 0.3) calc(c * 1.4) h / 0.9) 96%,
			transparent 100%
		);
		animation: original-rotate 3s linear infinite;
	}

	:global(.original-border::after) {
		content: '';
		position: absolute;
		z-index: -1;
		inset: 0;
		background: linear-gradient(
			to bottom,
			oklch(from var(--primary) calc(l + 0.35) calc(c * 0.4) h / 1),
			oklch(from var(--primary) calc(l + 0.25) calc(c * 0.6) h / 1)
		);
		border-radius: inherit;
	}

	:global(.dark .original-border::after) {
		background: linear-gradient(
			to bottom,
			oklch(from var(--primary) calc(l - 0.25) calc(c * 0.7) h / 1),
			oklch(from var(--primary) calc(l - 0.35) calc(c * 0.5) h / 1)
		);
	}

	:global(.dark .original-border::before) {
		background: conic-gradient(
			from var(--original-angle),
			transparent 0%,
			transparent 50%,
			oklch(from var(--primary) calc(l + 0.05) calc(c * 1.3) h / 0.2) 62%,
			oklch(from var(--primary) calc(l + 0.1) calc(c * 1.4) h / 0.5) 72%,
			oklch(from var(--primary) calc(l + 0.2) calc(c * 1.5) h / 0.85) 82%,
			oklch(from var(--primary) calc(l + 0.3) calc(c * 1.6) h / 1) 90%,
			oklch(from var(--primary) calc(l + 0.2) calc(c * 1.5) h / 0.85) 96%,
			transparent 100%
		);
		animation: original-rotate 3s linear infinite;
	}
