<script lang="ts">
	...
	// Sync previewImage with value prop when it changes (but not if user has made changes)
	$effect(() => {
		// Only update if user hasn't selected a new file or marked for deletion
		if (!selectedFile && !markedForDeletion) {
