import { db } from '../db';
import { organization, member, connection } from '../db/schema';
import { eq, and } from 'drizzle-orm';
import { error } from '@sveltejs/kit';
import { PLAN_LIMITS } from '../pricing/constants';
 ...
 * - Must not exceed maxMembersPerSpace limit
 */
export async function canInviteUsers(userId: string, spaceId: string): Promise<boolean> {
	...
	// Check if user has BYOI connection (Pro) or is on free trial
	const conn = await db.query.connection.findFirst({
		where: eq(connection.userId, space.createdBy),
		columns: { dbStatus: true, schemaInitialized: true }
	});

	const isPro = conn && conn.dbStatus === 'connected' && conn.schemaInitialized;
	const maxMembers = isPro ? PLAN_LIMITS.pro.maxMembersPerSpace : PLAN_LIMITS.free.maxMembersPerSpace;
