...
export async function sendContactFormEmail(options: {
	...
	if (!contactEmail) {
		throw new Error('CONTACT_EMAIL is not configured in .env');
	}

	const subject = `[Contact] ${options.subject} from ${options.name}`;
	const html = `
		<!DOCTYPE html>
		<html>
		<head>
			...
			<div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
				<table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
					<tr>
						...
						<td style="padding: 8px 12px;">${options.name}</td>
					</tr>
					<tr>
						<td style="padding: 8px 12px; font-weight: bold; color: #6b7280;">Email</td>
						<td style="padding: 8px 12px;"><a href="mailto:${options.email}" style="color: #10b981;">${options.email}</a></td>
					</tr>
					<tr>
						<td style="padding: 8px 12px; font-weight: bold; color: #6b7280;">Subject</td>
						<td style="padding: 8px 12px;">${options.subject}</td>
					</tr>
				</table>

				<hr style="border: none; border-top: 1px solid #e5e7eb; margin: 20px 0;">

				<h3 style="font-size: 14px; color: #6b7280; margin-bottom: 10px;">Message</h3>
				<div style="background: #f9fafb; padding: 16px; border-radius: 6px; white-space: pre-wrap; font-size: 15px;">
${options.message}
				</div>
			</div>

			<div style="text-align: center; margin-top: 20px; color: #9ca3af; font-size: 12px;">
				<p>Sent from the ${env.MAILGUN_FROM_NAME || 'FormTrap'} contact form</p>
			</div>
		</body>
		</html>
	`;
