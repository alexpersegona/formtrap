...
func InitLogger(appEnv string) {
	...
	// Hook to also write errors to error.log and send Discord alerts for fatal
	logger = logger.Hook(zerolog.HookFunc(func(e *zerolog.Event, level zerolog.Level, message string) {
		...
		// Send Discord alerts for fatal-level logs
		if level >= zerolog.FatalLevel {
			discord.SendAlert(discord.AlertOptions{
				Title:       "Fatal Error",
				Description: message,
				Severity:    "critical",
